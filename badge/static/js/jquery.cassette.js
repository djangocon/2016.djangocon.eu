...
( function(window, $, undefined) {
  ...
  $.Cassette.defaults   = {
    ...
    initialVolume : 0.7,
    seekSpeed: 10 // arbitrary value for the seeking speed - more is higher.
  };
  ...
  $.Cassette.prototype  = {
    ...
    _switchSides: function() {
      ...
      if(this.currentSide === 1) {
        this.currentSide = 2;
        var deg = '180';
        var showSide = _self.$tapeSideB;
        var hideSide = _self.$tapeSideA;
      } else {
        this.currentSide = 1;
        var deg = '0';
        var showSide = _self.$tapeSideA;
        var hideSide = _self.$tapeSideB;
      }
    ...
    _updateButtons: function( button ) {
      ...
      var buttonMapping = {
        'play': this.$cPlay,
        'rewind': this.$cRewind,
        'forward': this.$cForward,
        'stop': this.$cStop
      }

      for (var b in buttonMapping) {
        buttonMapping[b].removeClass(pressedClass);
      }

      // Stop button doesn't stay active
      if (button !== 'stop') {
        buttonMapping[button].addClass(pressedClass);
      }
    },
    ...
    _getSongInfoByTime: function(time) {
      ...
      var side = this._getSide().current;
      var playlist = side.getPlaylist();
      var len = side.getPlaylistCount();
      var cntTime = 0;
    ...
    _getPosTime: function() {
      var wleft = this.$wheelLeft.data('wheel') || 70;
      var wright = this.$wheelRight.data('wheel') || 0;
      var T = this._getSide().current.getDuration();

      return this.currentSide === 2 ? (T * wleft) / 70 : (T * wright) / 70;
    },
    _updateWheelValue: function(wheelVal) {
      this.$wheelLeft.data('wheel', wheelVal.left).css({
        'box-shadow': '0 0 0 ' + wheelVal.left + 'px black' 
      });
      this.$wheelRight.data('wheel', wheelVal.right).css({
    ...
    _setWheelAnimation: function(speed, mode) {
      ...
      if (mode === 'play' || mode === 'forward') {
        anim = (this.currentSide === 1) ? 'rotateRight' : 'rotateLeft';
      } else if(mode === 'rewind') {
        anim = (this.currentSide === 1) ? 'rotateLeft' : 'rotateRight';
      }

      var animValue = anim + ' ' + speed + ' linear infinite forwards';
      var animStyle = { 
        '-webkit-animation': animValue,
        '-moz-animation': animValue,
        '-o-animation': animValue,
        '-ms-animation': animValue,
        'animation': animValue
      };
    ...
    _timerinstance: function(start, time) {
      ...
      time += 100;  
      this.elapsed = Math.floor(time / 20) / (10 / this.options.seekSpeed);  
  ...
  $.Song.prototype = {
    ...
    loadMetadata: function() {
      ...
      return $.Deferred(
        function(dfd) {
          var $tmpAudio = $( '<audio/>' );
          var songsrc = _self.getSource(aux.getSupportedType());
  ...
  $.Sound.prototype = {
    ...
    getSource: function(type) {
      return '/static/audio/' + this.action + '.' + type;
    },
  ...
  $.fn.cassette = function(options) {
    if (typeof options === 'string') {
      ...
      this.each(function() {
        ...
        if (!instance) {
          logError("cannot call methods on cassette prior to initialization; attempted to call method '" + options + "'");
          return;
        }
